...
import hcmute.edu.vn.snaplearn.models.User; // Nhớ import model User
...
public class LoginViewModel extends ViewModel {
    ...
    // ĐỔI isLoginSuccess THÀNH loggedInUser ĐỂ TRUYỀN OBJECT ĐI
        ...
        authRepository.loginWithEmail(email, password, new AuthRepository.OnAuthListener() {
            ...
            public void onSuccess(FirebaseUser firebaseUser) {
                if (firebaseUser != null) {
                    // KHI XÁC THỰC THÀNH CÔNG, GỌI FIRESTORE ĐỂ LẤY OBJECT USER
                    userRepository.getUser(firebaseUser.getUid(), new UserRepository.OnUserFetchListener() {
                        @Override
                        public void onSuccess(User user) {
                            isLoading.setValue(false);
                            toastMessage.setValue("Đăng nhập thành công!");
                            // Kích hoạt LiveData này để Activity chuyển trang
                        ...
                        public void onError(String error) {
                            isLoading.setValue(false);
                            toastMessage.setValue("Lỗi tải thông tin: " + error);
                        }
                    });
                }
            }
